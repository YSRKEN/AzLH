...
namespace AzLH.ViewModels {
	internal class GameScreenSelectViewModel : IDisposable, INotifyPropertyChanged
	{
		...
		public GameScreenSelectViewModel(List<Rectangle> rectList, SelectGameWindowAction dg) {
			...
			this.dg = dg;
			// プロパティを設定
			PageInfoStr = RectIndex.Select(x => $"[{x + 1}/{rectList.Count}] {Utility.GetRectStr(rectList[x])}").ToReactiveProperty();
			GameWindowPage = RectIndex.Select(x => (BitmapSource)ScreenShotProvider.GetScreenBitmap(rectList[x]).ToImageSource()).ToReactiveProperty();
			// コマンドを設定
